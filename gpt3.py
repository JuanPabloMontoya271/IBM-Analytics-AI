...
```
"""
    question_answering = """Answer questions from the following table 
                            Q: {QUERY}
                            """
    ...
    @staticmethod
    def dataframes_summary(pruebas = None, name = "pruebas", ignore='^_') -> str:
        """
    ...
    def qa(self, query):
        ...
        prompt = self.dataframes_summary(pruebas)
        if not prompt:
            print('No dataframes found')
            return
        
        prompt += DataSloth.question_answering.format(QUERY=query)
